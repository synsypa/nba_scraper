...
,
    ...
    def fixSalaryName(self, old_df):
        """
        (NOTE THIS IS ONLY COMPLETE FOR 2014 SALARY DATA)
        Takes a salary dataframe from ESPN and processes mismatched player names
        to the version knonw in the NBA stats tables 
        Programmatic when possible
        Manually Otherwise
        """
        # Strip away player position
        old_df['PLAYER_NAME'] = old_df['PLAYER_NAME'].map(lambda x: x.rstrip(', [CSFPG]+'))
        # Strip Periods from Intials 
        old_df['PLAYER_NAME'] = old_df['PLAYER_NAME'].replace('\.(?!$)', '', regex=True)
         
        # Manual Adjustments  
        old_df['PLAYER_NAME'] = old_df['PLAYER_NAME'].replace('OJ Mayo', 'O.J. Mayo')
        old_df['PLAYER_NAME'] = old_df['PLAYER_NAME'].replace('JR Smith', 'J.R. Smith')
        old_df['PLAYER_NAME'] = old_df['PLAYER_NAME'].replace('Chuck Hayes', 'Charles Hayes')
        old_df['PLAYER_NAME'] = old_df['PLAYER_NAME'].replace('Louis Williams', 'Lou Williams')
        old_df['PLAYER_NAME'] = old_df['PLAYER_NAME'].replace('Otto Porter Jr.', 'Otto Porter')
        old_df['PLAYER_NAME'] = old_df['PLAYER_NAME'].replace('Luc Richard Mbah a Moute', 'Luc Mbah a Moute')
        old_df['PLAYER_NAME'] = old_df['PLAYER_NAME'].replace('DJ Augustin', 'D.J. Augustin')
        old_df['PLAYER_NAME'] = old_df['PLAYER_NAME'].replace('Luigi Datome', 'Gigi Datome')
        old_df['PLAYER_NAME'] = old_df['PLAYER_NAME'].replace('JJ Barea', 'Jose Juan Barea')
        old_df['PLAYER_NAME'] = old_df['PLAYER_NAME'].replace('Johnny O\'Bryant III', 'Johnny O\'Bryant')
        old_df['PLAYER_NAME'] = old_df['PLAYER_NAME'].replace('Toure\' Murry', 'Toure Murry')
        old_df['PLAYER_NAME'] = old_df['PLAYER_NAME'].replace('AJ Price', 'A.J. Price')
        old_df['PLAYER_NAME'] = old_df['PLAYER_NAME'].replace('Nene Hilario', 'Nene')
